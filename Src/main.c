...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
    uint8_t transformed_char = rx_data + 1;
    // Hacemos eco del caracter recibido
    HAL_UART_Transmit(&huart2, &transformed_char, 1, 10); // Enviar el byte recibido de vuelta
...
int main(void)
{
  ...
  char *welcome_msg = "Si inicio esta programa\r\n";
  ...
  while (1)
  {
    ...
    if (HAL_GetTick() - last_heartbeat_time >= 2000) // Cada 2000 ms
    {
    ...
    if (button_flag == 1)
    {
      ...
      // 2. Calcular cuándo apagarlo (1 segundos desde ahora)
      led_ext_off_time = HAL_GetTick() + 1000;

      // 3. Enviar mensaje por UART
      sprintf(tx_buffer, "Boton B1 presionado led encendido.\r\n");
    ...
    if (led_ext_off_time != 0 && HAL_GetTick() >= led_ext_off_time)
    {
      ...
      sprintf(tx_buffer, "LED APAGADO (se acabo el tiempo).\r\n");
      HAL_UART_Transmit(&huart2, (uint8_t *)tx_buffer, strlen(tx_buffer), 100);
